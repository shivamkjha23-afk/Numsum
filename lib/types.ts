...
export type PlatformStatus = "draft" | "submitted" | "under_review" | "needs_information" | "needs_more_info" | "onboarded" | "structured" | "onboarding_pending" | "onboarding_active" | "solution_path_created" | "active" | "completed" | "published" | "archived" | "rejected" | "closed" | "member_only" | "public" | "researching" | "competition" | "solved" | "scheduled" | "in_progress" | "cancelled" | "needs_follow_up";
...
export type CompetitionStatus = "draft" | "upcoming" | "published" | "open" | "active" | "registration_closed" | "submission_closed" | "closed" | "evaluation" | "evaluating" | "completed" | "results_declared" | "archived" | "cancelled";
...
export type UserProfileType = "msme_owner" | "student" | "researcher" | "engineer_professional" | "startup_technology_provider" | "consultant_industry_expert" | "other";
...
export interface ProblemStatement extends FirestoreEntity {
  ...
  adminVisibilityFlags?: { meetingNotesVisibleToMemberDefault?: boolean; [key: string]: unknown };
...
export interface ProblemImpactEstimate { monthlyMonetaryLoss?: string; timeLost?: string; wasteOrRejection?: string; productionDelay?: string; customersAffected?: string; safetyComplianceImpact?: string; expectedOutcome?: string; }
export interface ProblemImpactMetrics { monetarySaving?: string; timeSaved?: string; wasteReduction?: string; productivityGain?: string; clientsGained?: string; revenueImpact?: string; isPublic?: boolean; }
export interface ProblemMeetingNote extends FirestoreEntity { problemId: string; memberId?: string; membershipId?: string; title?: string; meetingTitle?: string; meetingDate?: DateLike; attendees?: string[]; questionsAnswers?: Array<{ question: string; answer: string }>; notes?: string; decisions?: string[]; actionItems?: string[]; nextSteps?: string; adminId?: string; adminName?: string; momPdfUrl?: string; visibleToMember?: boolean; createdByAdminId?: string; createdAt?: DateLike; updatedAt?: DateLike; }
export type QuestionnaireQuestionType = "text" | "textarea" | "number" | "select" | "multi_select" | "multiselect" | "checkbox" | "radio" | "date" | "file_or_link" | "file_link" | "rating" | "yes_no" | "dropdown" | "url" | "email";
export interface QuestionnaireQuestion { id: string; label: string; helpText?: string; type: QuestionnaireQuestionType; required?: boolean; options?: string[]; order?: number; placeholder?: string; adminOnly?: boolean; }
...
export type QuestionnaireUsageType = "problem_submission" | "onboarding_meeting" | "follow_up";
export interface QuestionnaireTemplate extends FirestoreEntity { title?: string; description?: string; purpose?: string; usageType?: QuestionnaireUsageType; category: ProblemCategory | string; sector?: string; industrySegment?: string; createdBy?: string; updatedBy?: string; isDefault?: boolean; status?: "draft" | "active" | "archived"; visibility?: "admin_only" | "public"; sections?: QuestionnaireSection[]; name?: string; questions: QuestionnaireQuestion[]; createdAt?: DateLike; updatedAt?: DateLike; }
export interface ProblemQuestionnaireResponse extends FirestoreEntity { problemId: string; memberId?: string; membershipId?: string; questionnaireId: string; questionnaireTitle?: string; usageType?: QuestionnaireUsageType; sector?: string; answers: Record<string, string>; createdByAdminId?: string; createdByAdminName?: string; visibleToMember?: boolean; createdAt?: DateLike; updatedAt?: DateLike; }
export interface ProblemMeetingNotePdf extends FirestoreEntity { problemId: string; meetingNoteId: string; memberId?: string; membershipId?: string; fileName: string; fileUrl?: string; storagePath?: string; generatedByAdminId?: string; generatedByAdminName?: string; visibleToMember?: boolean; createdAt?: DateLike; updatedAt?: DateLike; }

export interface PublicImpactMetrics { totalProblemsSubmitted: number; totalProblemsSolved: number; totalChallengesLaunched: number; totalChallengeParticipants: number; totalCaseStudiesPublished: number; totalCommunityPosts: number; totalMonetarySavings: string; totalTimeSaved: string; totalWasteReduction: string; totalProductivityGain: string; totalClientsGained: number; totalPublicReviews: number; }
export interface ProblemPublicReview extends FirestoreEntity { problemId: string; memberId: string; membershipId?: string; memberName?: string; organizationName?: string; sector?: string; problemTitle?: string; rating: number; reviewText: string; consentForPublicDisplay?: boolean; approvedForPublic?: boolean; moderationStatus?: "pending" | "approved" | "rejected" | "hidden"; adminTags?: string[]; reviewedBy?: string; reviewedAt?: DateLike; createdAt?: DateLike; updatedAt?: DateLike; }
export type CaseStudyStatus = "draft" | "published" | "archived";
export type CaseStudyClientDisplayMode = "hide_client_name" | "show_company_name" | "show_anonymized_company";
export interface CaseStudy extends FirestoreEntity { title: string; slug: string; summary?: string; sector?: string; tags?: string[]; linkedProblemId?: string; linkedChallengeId?: string; clientDisplayMode?: CaseStudyClientDisplayMode; companyName?: string; problemSection?: string; contextSection?: string; solutionSection?: string; implementationSection?: string; impactSection?: string; lessonsLearnedSection?: string; readTime?: string; featuredMetric?: string; status?: CaseStudyStatus; approvedForPublic?: boolean; upvoteCount?: number; commentCount?: number; createdByAdminId?: string; createdByAdminName?: string; createdAt?: DateLike; updatedAt?: DateLike; publishedAt?: DateLike; }
export interface CaseStudyComment extends FirestoreEntity { caseStudyId: string; memberId: string; membershipId?: string; memberName?: string; profileType?: string; body: string; status?: "pending" | "approved" | "hidden" | "rejected"; createdAt?: DateLike; updatedAt?: DateLike; }
export interface CaseStudyUpvote extends FirestoreEntity { caseStudyId: string; memberId: string; membershipId?: string; createdAt?: DateLike; }
...
export interface AdminAuditLog extends FirestoreEntity { actionType: string; targetUserId?: string; targetMembershipId?: string; targetEmail?: string; previousRole?: Role; newRole?: Role; previousStatus?: string; newStatus?: string; performedByUserId?: string; performedByMembershipId?: string; performedByName?: string; reason?: string; createdAt?: DateLike; }
...
export type ChallengeParticipationStatus = "registered" | "withdrawn" | "active";
export type ChallengeTeamStatus = "draft" | "active" | "inviting" | "locked" | "withdrawn";
export type ChallengeInviteStatus = "pending" | "accepted" | "rejected";
export type ChallengeSubmissionStatus = "draft" | "submitted" | "withdrawn";
export interface ChallengeParticipation extends FirestoreEntity { challengeId: string; memberId: string; membershipId?: string; memberName?: string; profileType?: string; status?: ChallengeParticipationStatus; createdAt?: DateLike; updatedAt?: DateLike; }
export interface ChallengeTeam extends FirestoreEntity { challengeId: string; teamName: string; leaderMemberId: string; leaderMembershipId?: string; memberIds: string[]; memberMembershipIds?: string[]; invitedMembershipIds?: string[]; invitedMemberIds?: string[]; acceptedMemberIds?: string[]; rejectedMemberIds?: string[]; maxTeamSize?: number; status?: ChallengeTeamStatus; createdAt?: DateLike; updatedAt?: DateLike; }
export interface ChallengeTeamInvite extends FirestoreEntity { challengeId: string; teamId: string; teamName?: string; challengeTitle?: string; invitedMemberId: string; invitedMembershipId?: string; invitedMemberName?: string; invitedByMemberId: string; invitedByMembershipId?: string; invitedByName?: string; status?: ChallengeInviteStatus; createdAt?: DateLike; updatedAt?: DateLike; respondedAt?: DateLike; }
export interface ChallengeSubmission extends FirestoreEntity { challengeId: string; teamId: string; submittedByMemberId: string; submittedByMembershipId?: string; solutionTitle: string; solutionSummary: string; detailedApproach?: string; expectedImpact?: string; implementationPlan?: string; evidenceLinks?: string[]; demoVideoLink?: string; pitchDeckLink?: string; declarationAccepted?: boolean; status?: ChallengeSubmissionStatus; submittedAt?: DateLike; createdAt?: DateLike; updatedAt?: DateLike; locked?: boolean; evaluationStatus?: string; marks?: number; rank?: number; adminComments?: string; evaluatedBy?: string; evaluatedAt?: DateLike; resultApproved?: boolean; }
export interface ChallengeEvaluation extends FirestoreEntity { challengeId: string; submissionId: string; teamId?: string; evaluatorAdminId?: string; evaluatorAdminName?: string; marks?: number; rubricScores?: Record<string, number>; comments?: string; status?: "pending" | "evaluated" | "needs_revision" | "disqualified"; createdAt?: DateLike; updatedAt?: DateLike; }
export interface ChallengeResultRanking { submissionId: string; teamId?: string; rank: number; marks?: number; title?: string; }
export interface ChallengeResult extends FirestoreEntity { challengeId: string; winnerSubmissionId?: string; winnerTeamId?: string; ranking?: ChallengeResultRanking[]; declaredByAdminId?: string; declaredByAdminName?: string; approvedForPublic?: boolean; declaredAt?: DateLike; createdAt?: DateLike; updatedAt?: DateLike; }
