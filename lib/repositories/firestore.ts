...
import type {
  ...
  ExecutionWorkItem, ExecutionReview, MeetingRecord, ActionItem, DecisionRecord, EvidenceRecord,
} from "@/lib/types";
...
export async function completeExecutionWorkItem(id: string, completionNotes = "", evidenceLinks: string[] = []) { const item = await getRecord<ExecutionWorkItem>(COLLECTIONS.executionWorkItems, id); if (!item) throw new Error("Work item not found"); if (item.evidenceRequired && !completionNotes && !evidenceLinks.length && !(item.evidenceLinks || []).length) throw new Error("Completion notes or evidence are required"); await updateRecord(COLLECTIONS.executionWorkItems, id, { status: "completed", completedAt: serverTimestamp(), completionNotes, evidenceLinks: [...(item.evidenceLinks || []), ...evidenceLinks], updatedBy: uid() }); const all = await getExecutionWorkItemsForObjective(item.objectiveTargetId); const total = Math.max(all.length, 1); const completed = all.filter((w) => w.id === id || w.status === "completed").length; await updateObjectiveProgress(item.objectiveTargetId, Math.round((completed / total) * 100), "Progress recalculated from completed execution work items.").catch(()=>{}); }
...
export const completeActionItem = (id: string, completionNotes = "", evidenceLinks: string[] = []) => updateRecord(COLLECTIONS.actionItems, id, { status: "completed", completedAt: serverTimestamp(), completionNotes, evidenceLinks: arrayUnion(...evidenceLinks), updatedBy: uid() });
