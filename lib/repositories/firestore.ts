import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, limit, orderBy, query, serverTimestamp, setDoc, updateDoc, type DocumentData, type QueryConstraint, type WithFieldValue } from "firebase/firestore";
import { cache } from "react";
import { db } from "@/lib/firebase";
import type { Challenge, Organization, Questionnaire, SystemStats, UserProfile } from "@/lib/types";

export const COLLECTIONS = { users: "users", organizations: "organizations", challenges: "challenges", systemStats: "system_stats" } as const;
...
export const getPlatformStats = cache(async (): Promise<SystemStats | null> => {
  const snap = await getDoc(doc(db, COLLECTIONS.systemStats, "platform"));
  return snap.exists() ? withId<SystemStats>(snap) : null;
});

export async function createChallengeFromProblem(data: Omit<Challenge, "id" | "createdAt" | "updatedAt" | "status">) {
  return createRecord<Challenge>(COLLECTIONS.challenges, { ...data, status: "in_review", createdAt: serverTimestamp(), updatedAt: serverTimestamp() } as WithFieldValue<Challenge>);
}
