...
export async function createChallengeTeam(data: Omit<ChallengeTeam, "id" | "createdAt" | "updatedAt">) { const payload = { ...data, memberIds: Array.from(new Set(data.memberIds || [data.leaderMemberId])), acceptedMemberIds: Array.from(new Set(data.acceptedMemberIds || [data.leaderMemberId])), memberMembershipIds: Array.from(new Set(data.memberMembershipIds || [data.leaderMembershipId || ""].filter(Boolean))), invitedMembershipIds: data.invitedMembershipIds || [], invitedMemberIds: data.invitedMemberIds || [], rejectedMemberIds: data.rejectedMemberIds || [], status: data.status || "active" } as ChallengeTeam; const ref = await createRecord<ChallengeTeam>(COLLECTIONS.challengeTeams, { ...payload, createdAt: serverTimestamp(), updatedAt: serverTimestamp() } as never); return { ...payload, id: ref.id } as ChallengeTeam; }
...
export async function respondToChallengeInvite(invite: ChallengeTeamInvite, status: "accepted" | "rejected", memberId: string) { if (invite.invitedMemberId !== memberId) throw new Error("You can only respond to your own invite."); await updateRecord(COLLECTIONS.challengeTeamInvites, invite.id, { status, respondedAt: serverTimestamp() }); const team = await getRecord<ChallengeTeam>(COLLECTIONS.challengeTeams, invite.teamId); if (team) await updateRecord(COLLECTIONS.challengeTeams, invite.teamId, status === "accepted" ? { memberIds: arrayUnion(memberId), acceptedMemberIds: arrayUnion(memberId), memberMembershipIds: arrayUnion(invite.invitedMembershipId), invitedMemberIds: arrayRemove(memberId), updatedAt: serverTimestamp() } : { rejectedMemberIds: arrayUnion(memberId), invitedMemberIds: arrayRemove(memberId), updatedAt: serverTimestamp() }); }
export async function createOrUpdateChallengeSubmission(data: Omit<ChallengeSubmission, "id" | "createdAt" | "updatedAt"> & { id?: string }) { const { id, marks, rank, adminComments, evaluatedBy, evaluatedAt, resultApproved, evaluationStatus, ...memberData } = data; void marks; void rank; void adminComments; void evaluatedBy; void evaluatedAt; void resultApproved; void evaluationStatus; const isFinal = memberData.status === "submitted"; const payload = { ...memberData, evidenceLinks: memberData.evidenceLinks || [], locked: isFinal, ...(isFinal ? { submittedAt: serverTimestamp() } : {}), updatedAt: serverTimestamp() }; if (id) { await updateRecord(COLLECTIONS.challengeSubmissions, id, payload as Record<string, unknown>); return { ...memberData, id } as ChallengeSubmission; } const ref = await createRecord<ChallengeSubmission>(COLLECTIONS.challengeSubmissions, { ...payload, createdAt: serverTimestamp() } as never); return { ...memberData, id: ref.id } as ChallengeSubmission; }
...
export async function getPublicProblemReviews() { return listCollection<ProblemPublicReview>(COLLECTIONS.problemPublicReviews, [where("approvedForPublic", "==", true), where("consentForPublicDisplay", "==", true), limit(20)]).catch(() => []); }
...
export const getAdminCaseStudies = cache(async () => byCreatedAtDesc(await listCollection<CaseStudy>(COLLECTIONS.caseStudies, [limit(300)]).catch(() => [])));
export const getCaseStudyById = cache(async (id: string) => getRecord<CaseStudy>(COLLECTIONS.caseStudies, id));
export const getPublicAdminCaseStudies = cache(async () => byCreatedAtDesc(await listCollection<CaseStudy>(COLLECTIONS.caseStudies, [where("status", "==", "published"), where("approvedForPublic", "==", true), limit(100)]).catch(() => [])));
export const getPublicAdminCaseStudyBySlug = cache(async (slug: string) => (await listCollection<CaseStudy>(COLLECTIONS.caseStudies, [where("slug", "==", slug), where("status", "==", "published"), where("approvedForPublic", "==", true), limit(1)]).catch(() => []))[0] || null);
export async function createAdminCaseStudy(data: Partial<CaseStudy>, actor?: Pick<UserProfile, "id" | "uid" | "fullName" | "name" | "email"> | null) { const payload = { ...data, title: data.title || "Untitled case study", slug: data.slug || String(data.title || "case-study").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""), tags: data.tags || [], status: data.status || "draft", approvedForPublic: Boolean(data.approvedForPublic), upvoteCount: data.upvoteCount || 0, commentCount: data.commentCount || 0, createdByAdminId: actor?.uid || actor?.id || "admin", createdByAdminName: actor?.fullName || actor?.name || actor?.email || "Admin", ...(data.status === "published" && data.approvedForPublic ? { publishedAt: serverTimestamp() } : {}), createdAt: serverTimestamp(), updatedAt: serverTimestamp() }; const ref = await createRecord<CaseStudy>(COLLECTIONS.caseStudies, payload as never); return { ...(data as CaseStudy), id: ref.id, title: payload.title, slug: payload.slug } as CaseStudy; }
export async function updateAdminCaseStudy(id: string, patch: Partial<CaseStudy>) { return updateRecord(COLLECTIONS.caseStudies, id, { ...patch, tags: patch.tags || [], ...(patch.status === "published" && patch.approvedForPublic ? { publishedAt: serverTimestamp() } : {}) } as Record<string, unknown>); }
export const getCaseStudyCommentsAdmin = cache(async (caseStudyId: string) => byCreatedAtDesc(await listCollection<CaseStudyComment>(COLLECTIONS.caseStudyComments, [where("caseStudyId", "==", caseStudyId), limit(200)]).catch(() => [])));
export async function moderateCaseStudyComment(id: string, status: NonNullable<CaseStudyComment["status"]>) { const existing = await getRecord<CaseStudyComment>(COLLECTIONS.caseStudyComments, id).catch(() => null); await updateRecord(COLLECTIONS.caseStudyComments, id, { status }); if (existing?.caseStudyId && status === "approved" && existing.status !== "approved") await updateRecord(COLLECTIONS.caseStudies, existing.caseStudyId, { commentCount: increment(1) }).catch(() => undefined); if (existing?.caseStudyId && status !== "approved" && existing.status === "approved") await updateRecord(COLLECTIONS.caseStudies, existing.caseStudyId, { commentCount: increment(-1) }).catch(() => undefined); }
export async function deleteCaseStudyComment(id: string) { return deleteRecord(COLLECTIONS.caseStudyComments, id); }
export async function getApprovedCaseStudyComments(caseStudyId: string) { return listCollection<CaseStudyComment>(COLLECTIONS.caseStudyComments, [where("caseStudyId", "==", caseStudyId), where("status", "==", "approved"), limit(100)]).catch(() => []); }
export const getCaseStudyComments = getApprovedCaseStudyComments;
export async function getMemberPendingCaseStudyComments(caseStudyId: string, memberId: string) { return listCollection<CaseStudyComment>(COLLECTIONS.caseStudyComments, [where("caseStudyId", "==", caseStudyId), where("memberId", "==", memberId), where("status", "==", "pending"), limit(20)]).catch(() => []); }
...
// Deprecated fallback only: MOM PDFs are generated on demand from problem_meeting_notes; no permanent PDF/file storage is used in active UI.
...
export const listPublicDiscussionThreads = cache(async () => byCreatedAtDesc((await listCollection<DiscussionThread>(COLLECTIONS.discussionThreads, [where("visibility", "==", "public"), where("status", "==", "open"), limit(100)])).filter((t) => !t.moderationStatus || ["clean", "approved"].includes(t.moderationStatus))));
