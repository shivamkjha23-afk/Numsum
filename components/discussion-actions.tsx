...
import type { DiscussionCategory, DiscussionThread, DiscussionVisibility } from "@/lib/types";
...
export function NewDiscussionForm() {
  ...
  async function submit(formData: FormData) {
    ...
    setMessage(visibility === "public" ? "Discussion submitted for public moderation review." : "Discussion created.");
  }
  return <form action={submit} className="grid gap-3"><input name="title" required placeholder="Discussion title" className={input}/><textarea name="body" required placeholder="Share practical context, questions, or field observations. Do not include confidential MSME data." className={`${input} h-36`}/><div className="grid gap-3 md:grid-cols-2"><select name="category" className={input}>{["question","field_observation","technical_discussion","research_discussion","competition_discussion","team_coordination","msme_need","announcement"].map((c)=><option key={c} value={c}>{c.replaceAll("_"," ")}</option>)}</select><select name="visibility" defaultValue="members" className={input}><option value="members">Members</option><option value="public">Public (moderated)</option><option value="private_problem">Private problem</option><option value="private_team">Private team</option></select></div><div className="grid gap-3 md:grid-cols-2"><select name="scopeType" defaultValue="general" className={input}>{["general","problem","competition","team","knowledge","research","sop","pilot"].map((s)=><option key={s}>{s}</option>)}</select><input name="scopeId" placeholder="Optional related entity ID" className={input}/></div><input name="tags" placeholder="Tags (comma-separated)" className={input}/><button className="rounded-full bg-white px-5 py-3 font-semibold text-black">Create discussion</button>{message && <p className="text-sm text-white/60">{message}</p>}</form>;
}
