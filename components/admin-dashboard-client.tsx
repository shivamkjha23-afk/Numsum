...
import { getAdminApplications, getAdminInbox, getAllChallenges, getCollaborationRequests, getCommunityPosts, getCompetitions, getInitializationStatus, getInternalThreads, getKnowledgeAssets, getOrganizations, getPlatformStats, getQuestionnaireTemplates, getResearchPosts, getTeamMembers, getUsers, initializePlatform } from "@/lib/repositories/firestore";
import type { AdminApplication, AdminInboxItem, Challenge, CollaborationRequest, CommunityPost, Competition, InitializationStatus, InternalThread, KnowledgeAsset, Organization, QuestionnaireTemplate, ResearchPost, SystemStats, TeamMember, UserProfile } from "@/lib/types";
...
function AdminDataLoader() {
  ...
  useEffect(() => { let mounted = true; initializePlatform().then(() => Promise.all([getAdminInbox(), getAllChallenges(100), getOrganizations(100), getUsers(100), getPlatformStats(), getAdminApplications(), getQuestionnaireTemplates(), getInternalThreads(), getResearchPosts(), getKnowledgeAssets(), getCompetitions(), getCommunityPosts(), getTeamMembers(), getCollaborationRequests(), getInitializationStatus()])).then(([adminInbox, challenges, organizations, users, stats, adminApplications, questionnaireTemplates, internalThreads, researchPosts, knowledgeAssets, competitions, communityPosts, teamMembers, collaborationRequests, initializationStatus]) => { if (mounted) setData({ adminInbox, challenges, organizations, users, stats, adminApplications, questionnaireTemplates, internalThreads, researchPosts, knowledgeAssets, competitions, communityPosts, teamMembers, collaborationRequests, initializationStatus }); }).catch((err) => { if (mounted) setError(err instanceof Error ? err.message : "Unable to load admin data."); }); return () => { mounted = false; }; }, []);
