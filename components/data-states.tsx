...
export function LoadingState({ label = "Loading data" }: { label?: string }) {
  return <Card className="animate-pulse text-white/60" role="status" aria-live="polite" aria-busy="true">{label}…</Card>;
}
...
export function EmptyState({ title, message = "Be among the first contributors" }: { title?: string; message?: string }) {
  return <Card className="text-white/60" aria-live="polite">{title && <h3 className="font-display text-2xl text-white">{title}</h3>}<p className={title ? "mt-2" : ""}>{message}</p></Card>;
}
