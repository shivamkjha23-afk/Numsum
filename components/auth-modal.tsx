...
export function AuthModal({
  ...
  async function continueWithEmail(formData: FormData) {
    setError("");
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/70 p-4 backdrop-blur">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#07111f] p-6 shadow-2xl">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-2 text-xs text-white/45">
              Enter your email to continue. If this is your first time, a member account is created automatically.
            </p>
          </div>
        ...
        <button className="mt-6 w-full rounded-full bg-white px-5 py-3 font-semibold text-navy" onClick={continueWithGoogle}>
          Continue with Google
        </button>

        <form action={continueWithEmail} className="mt-4 grid gap-3">
          <input name="email" type="email" required placeholder="Email" className="rounded-xl border border-white/10 bg-black/30 p-3" />
          <input name="password" type="password" required placeholder="Password" className="rounded-xl border border-white/10 bg-black/30 p-3" />
          <button className="rounded-full bg-blue-400 px-5 py-3 font-semibold text-navy">Continue with Email</button>
        </form>

        {error && (
          <p className="mt-4 rounded-xl border border-red-300/20 bg-red-500/10 p-3 text-sm text-red-100">{error}</p>
        )}
      </div>
    </div>
  );
