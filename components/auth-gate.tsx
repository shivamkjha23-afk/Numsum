...
export function AuthGate({ children, label = "Please sign in to continue", adminOnly = false, requireComplete = true }: { children: ReactNode; label?: string; adminOnly?: boolean; requireComplete?: boolean }) {
  ...
  useEffect(() => {
    if (loading || !user || !authReady || !requireComplete || profileComplete || pathname === "/profile/complete") return;
  ...
  if (requireComplete && !profileComplete && pathname !== "/profile/complete") return <LoadingState label="Redirecting to complete your profile" />;
  return <>{children}</>;
}
