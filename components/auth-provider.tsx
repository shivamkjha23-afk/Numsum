...
type AuthContextValue = { user: User | null; profile: UserProfile | null; loading: boolean; requestAuth: (options?: NonNullable<PendingAuth>) => void; closeAuth: () => void; };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => onAuthStateChanged(auth, async (next) => {
    ...
    try { setProfile(await ensureUserProfile(next)); } catch { setProfile(null); }
    setLoading(false);
  }), []);
  ...
  const value = useMemo(() => ({ user, profile, loading, requestAuth, closeAuth }), [user, profile, loading, requestAuth, closeAuth]);
