...
import { getAdminProblemWorkspaceMetrics, getSubmittedProblemStatements, getGovernanceMetrics } from "@/lib/repositories/firestore";
...
export default async function Admin() {
  ...
  const governanceMetrics = await getGovernanceMetrics().catch(() => null);
  const fallback = {
  ...
  return <main className="min-h-screen bg-navy px-6 py-10"><h1 className="font-display text-5xl">Admin Dashboard</h1><p className="mt-3 text-white/60">Moderate problem statements, workspace activity, inbox items, applications, research, knowledge, competitions, organizations, users, team members and platform statistics.</p>{governanceMetrics && <Card className="mt-6"><p className="text-sm uppercase tracking-[.25em] text-blue-300">Governance Metrics</p><dl className="mt-4 grid gap-2 md:grid-cols-4 text-sm text-white/65">{Object.entries(governanceMetrics).map(([label,value]) => <div key={label} className="rounded-xl bg-white/[0.03] p-3"><dt>{label.replace(/([A-Z])/g, " $1")}</dt><dd className="text-lg text-white">{String(value)}</dd></div>)}</dl><div className="mt-4"><Button href="/admin/governance">Open Governance</Button></div></Card>}<div className="mt-6 grid gap-4 md:grid-cols-[360px_1fr]"><Card><p className="text-sm uppercase tracking-[.25em] text-blue-300">Problem Workspace Metrics</p><p className="mt-3 text-3xl text-white">{m.totalProblems}</p><p className="mt-2 text-sm text-white/60">Central operating counts for problem workspaces and linked activity.</p><div className="mt-5"><Button href="/admin/problems">Open Problem Review</Button></div><dl className="mt-5 grid grid-cols-2 gap-2 text-sm text-white/65">{cards.slice(1).map(([label, count]) => <div key={label} className="rounded-xl bg-white/[0.03] p-3"><dt>{label}</dt><dd className="text-lg text-white">{count}</dd></div>)}</dl></Card><div><AdminDashboardClient /></div></div></main>;
}
