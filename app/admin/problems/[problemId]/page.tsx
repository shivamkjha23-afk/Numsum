...
import { COLLECTIONS, createProblemMeetingNote, createProblemMeetingNotePdf, createProblemQuestionnaireResponse, getActiveQuestionnaireTemplates, getProblemMeetingNotePdfsAdmin, getProblemMeetingNotesAdmin, getProblemQuestionnaireResponsesAdmin, getProblemReviewForMember, getProblemStatementById, getRecord, moderateProblemPublicReview, updateProblemImpactMetrics, updateRecord } from "@/lib/repositories/firestore";
import { buildMomPdfFileName, buildMomPdfHtml } from "@/lib/mom-pdf";
import type { ProblemImpactMetrics, ProblemMeetingNote, ProblemMeetingNotePdf, ProblemPublicReview, ProblemQuestionnaireResponse, ProblemStatement, ProblemStatementStatus, QuestionnaireTemplate, UserProfile } from "@/lib/types";
...
export default function AdminProblemDetailPage() {
  ...
  const [pdfs, setPdfs] = useState<ProblemMeetingNotePdf[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [loadedOwner, loadedNotes, loadedReview, loadedTemplates, loadedResponses, loadedPdfs] = await Promise.all([
        ...
        getProblemMeetingNotePdfsAdmin(loaded.id),
      ]);
  ...
  async function createMomMetadata(note: ProblemMeetingNote) {
    if (!problem || !user) return;
    const fileName = buildMomPdfFileName(problem, note);
    await createProblemMeetingNotePdf({ problemId: problem.id, meetingNoteId: note.id, memberId: ownerId, membershipId: problem.membershipId || owner?.membershipId, fileName, fileUrl: "", storagePath: `problem_meeting_note_pdfs/${problem.id}/${note.id}/${fileName}`, generatedByAdminId: user.uid, generatedByAdminName: adminName, visibleToMember: Boolean(note.visibleToMember) });
    setMessage("MOM/PDF metadata saved. Storage upload can be attached when Firebase Storage is configured.");
    await load();
  }
  function previewMom(note: ProblemMeetingNote) { if (!problem) return; const win = window.open("", "_blank"); if (win) { win.document.write(buildMomPdfHtml(problem, note, owner)); win.document.close(); } }
  function downloadMom(note: ProblemMeetingNote) { if (!problem) return; const blob = new Blob([buildMomPdfHtml(problem, note, owner)], { type: "application/pdf" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = buildMomPdfFileName(problem, note); a.click(); URL.revokeObjectURL(url); }
  ...
  if (error) return <main className="min-h-screen bg-navy px-6 py-10"><EmptyState title="Problem unavailable" message={error} /></main>;
  if (!problem) return <main className="min-h-screen bg-navy px-6 py-10"><EmptyState title="Problem not found" /></main>;
