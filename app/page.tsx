...
import { getOrCreatePlatformStats, getProblemStatements } from "@/lib/repositories/firestore";

async function HomeStats() {
  try {
    const stats = await getOrCreatePlatformStats();
...
export default function Home() {
  return <main className="overflow-hidden bg-[radial-gradient(circle_at_top,#0b3b78_0,#02050a_42%)]"><section className="mx-auto flex min-h-[58vh] max-w-7xl flex-col justify-center px-6 py-16"><div><p className="mb-5 text-sm uppercase tracking-[.35em] text-blue-300">Futuristic Industrial Intelligence</p><HeroTypewriter /><p className="mt-6 max-w-2xl text-xl text-white/70">Connecting industry problem statements with talent, innovation and practical solutions.</p><div className="mt-9 flex flex-wrap gap-3"><ProtectedLink href="/submit-problem" className="rounded-full bg-white px-5 py-3 text-sm font-semibold text-black transition hover:bg-blue-100">Submit Problem</ProtectedLink><Button href="/community" variant="secondary">Join Community</Button><Button href="/problem-statements" variant="secondary">Explore Problem Statements</Button><Button href="/msme-intelligence" variant="secondary">MSME Intelligence</Button></div></div></section><section className="mx-auto max-w-7xl px-6 pb-20"><NetworkHero /></section><section className="mx-auto max-w-7xl px-6 py-20"><h2 className="font-display text-4xl">Live Problem Statement Board</h2><Suspense fallback={<LoadingState label="Loading problem statements" />}><ChallengeBoard /></Suspense></section><section className="mx-auto max-w-7xl px-6 py-20"><h2 className="font-display text-4xl">Platform Statistics</h2><div className="mt-8"><Suspense fallback={<LoadingState label="Loading statistics" />}><HomeStats /></Suspense></div></section></main>;
}
