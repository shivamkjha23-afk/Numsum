...
export default function NewDiscussion() {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Unable to create discussion.",
      );
    }
  }
  return (
    <AuthGate>
      <main className="min-h-screen bg-navy px-6 py-10">
        <h1 className="font-display text-5xl">Create Discussion Post</h1>
        <Card className="mt-8">
          <p className="text-white/60">
            Authenticated members can create structured discussions, comments,
            replies, bookmarks and upvotes around platform objects.
          </p>
          <form action={submit} className="mt-4 grid gap-3">
            ...
            <textarea
              ...
              className="h-32 rounded-xl border border-white/10 bg-black/30 p-3"
            />
            <DocumentSelector
          ...
          {message && <p className="mt-3 text-white/60">{message}</p>}
        </Card>
      </main>
    </AuthGate>
  );
