...
function CreateTeamForm() {
  ...
  return <Card className="mt-8"><p className="text-white/60">Create a team, select existing users, or add invite emails for collaborators who have not joined yet.</p><form action={submit} className="mt-4 grid gap-4"><input name="name" required placeholder="Team Name" className="rounded-xl border border-white/10 bg-black/30 p-3" /><input name="focusArea" placeholder="Focus Area" className="rounded-xl border border-white/10 bg-black/30 p-3" /><input name="skills" placeholder="Required Skills (comma-separated)" className="rounded-xl border border-white/10 bg-black/30 p-3" /><input name="competitionId" placeholder="Challenge ID (optional)" className="rounded-xl border border-white/10 bg-black/30 p-3" /><div><label className="text-sm text-white/60">Find members by email or name</label><div className="mt-2 flex gap-2"><input value={query} onChange={(e) => search(e.target.value)} placeholder="person@example.com" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 p-3" /><button type="button" onClick={addInvite} className="rounded-xl border border-white/10 px-4">Invite email</button></div>{matches.length > 0 && <div className="mt-2 rounded-2xl border border-white/10 bg-black/30 p-2">{matches.map((u) => <button type="button" key={u.id} onClick={() => { setSelected((rows) => [...rows, u]); setQuery(""); }} className="block w-full rounded-xl p-2 text-left hover:bg-white/10">{u.name || u.email} <span className="text-white/40">{u.email}</span></button>)}</div>}</div><div className="flex flex-wrap gap-2">{selected.map((u) => <span key={u.id} className="rounded-full bg-blue-500/20 px-3 py-1 text-blue-100">{u.name || u.email}</span>)}{invites.map((email) => <span key={email} className="rounded-full bg-amber-500/20 px-3 py-1 text-amber-100">Invite: {email}</span>)}</div><button className="rounded-full bg-white px-5 py-3 font-semibold text-black">Create Team</button></form>{message && <p className="mt-3 text-white/60">{message}</p>}</Card>;
}
