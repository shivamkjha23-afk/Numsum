...
export default function SignIn() {
  ...
  async function submit(formData: FormData) {
    setError("");
    try { await signInWithEmailAndPassword(auth, String(formData.get("email")), String(formData.get("password"))); router.push(new URLSearchParams(window.location.search).get("returnTo") || "/dashboard"); } catch (e) { setError(authErrorMessage(e)); }
  }
  async function google() {
    setError("");
    try { await signInWithPopup(auth, new GoogleAuthProvider()); router.push(new URLSearchParams(window.location.search).get("returnTo") || "/dashboard"); } catch (e) { setError(authErrorMessage(e)); }
  }
